            ...
            <div class="col-sm-7 row">
                ...
                <div class="col-lg-9 col-sm-8">
                    <div class="segmented-control" style="width:100%;border:1px solid #ccc">
                        <input v-for="option in hospitals" type="radio" id="hospitalRadio{{$index}}" v-model='hospital' name="_hospital" :value="option">
                        <label v-for="option in hospitals" for="hospitalRadio{{$index}}" @click="clickLabel(option, $event)" :class="{active1: hospital == option}">{{option}}</label>
                    </div>
                </div>
            </div>
        ...
        <div class="form-group" style="margin-top:10px">
            <div class="col-sm-7 row">
                <div class="col-lg-9 col-lg-offset-3 col-sm-8 col-sm-offset-4">
                    <button v-privilege.literal="数据库-量表-添加和修改" :disabled='iserror' type="submit" class="btn btn-success btn-save" @click="save($event)">保存</button>
                </div>
            </div>
        </div>
</div>
            ...
            <thead>
                ...
                <tbody>
                    <tr v-for="checkup in data.checkups">
                        ...
                        <td v-for="xquestion in data.xquestions">{{{getContent(xquestion, checkup)}}}</td>
                        <td>
                            <a v-privilege.literal="数据库-量表-添加和修改" href="javascript:" @click="modifyCheckup(checkup, $event)">修改</a>&nbsp;&nbsp;
                            <a v-privilege.literal="数据库-量表-删除" href="javascript:" @click="deleteCheckup(checkup, $event)">删除</a>
                        </td>
                    </tr>
                </tbody>
        </table>
...
module.exports = {
        ...
        save: function(e) {
            ...
            if (!this.ismodify) { //新增
                if (this.isExist(data.checkdate)) {
                    var that = this;
                    this.$dispatch('show-prompt', '该日期已经存在检查报告，确定要覆盖吗?', function() {
                        //确定
                        that.doSave(data, e);
                    }, function() {
                        //取消
                ...
                if (this.data.hasOwnProperty('checkups')) {
                    for (var i = 0; i < this.data.checkups.length; i++) {
                        ...
                        if (data.checkdate == checkup.check_date) {
                            ...
                            this.$dispatch('show-alert', '您所选择的日期已经存在量表数据，请修改对应日期的量表，谢谢！');
                            return false;
                        }
                    }
                }
    ...
    events: {
        ...
        'edss-fs-change': function(questionName, fs) {
            ...
            this.$broadcast('edss-notify', this.edssScore);
        },
        ...
        'edss-hide-popover': function() {
            this.$broadcast('edss-hide-popover-child')
        },
        'bsa-wh-change': function(name, value) {
            ...
            this.$broadcast('bsa-notify', bsa);
        },
        'show-component': function(ename) {
            this.$broadcast('show-component-notify', ename);
        },
        'hide-component': function(ename) {
            this.$broadcast('hide-component-notify', ename);
        }
    },
    ready: function() {
