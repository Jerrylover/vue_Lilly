<template>
    ...
    <div class="container-fluid content">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tr v-for="patient in patientlist" @click="setselectstatus(patient, $index)">
                <td>
                    <div class="checkbox checkbox-inline checkbox-info">
        ...
        methods: {
            selectallpatient: function() {
                ...
                for (var i = 0; i < this.patientlist.length; i++) {
                    ...
                    this.patientlist.$set(i, this.patientlist[i]);
                }
            },
            dosearch: function() {
                var self = this;
            ...
            importone: function(patient) {
                ...
                $.ajax({
                    ...
                    if (response.errno == 0) {
                        ...
                        self.$dispatch('show-popup', "导入成功", function(){
            ...
            removeone: function(patient) {
                ...
                $.ajax({
                    ...
                    if (response.errno == 0) {
                        var data = response.data;
                        self.$dispatch('show-popup', '导出成功', function(){
                            self.selectallstatus = false;
                            self.fetchData();
                        })
                    }else {
            ...
            importall: function() {
                var self = this;
                this.patientids = [];
                self.$dispatch('show-prompt',"确定将所选患者全部导入吗？", function(){
            ...
            removeall: function() {
                var self = this;
                this.patientids = [];
                self.$dispatch('show-prompt',"确定将所选患者全部导出吗？", function(){
                    for (var i = 0; i < self.patientlist.length; i++) {
                        if(self.patientlist[i].selectstatus == true) {
                            self.patientids.push(self.patientlist[i].id);
                        }
            ...
            setselectstatus: function(patient, index){
                ...
                this.patientlist.$set(index, patient);
