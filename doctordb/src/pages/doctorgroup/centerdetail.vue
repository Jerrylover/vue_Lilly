    ...
    <div class="container-fluid content">
        <div class="row" style="border-bottom: 1px solid #ccc; padding-bottom: 0px">
            ...
            <div style="display: inline-block;">
                ...
                <a v-dg-privilege="dg_project.id + '-2 |' + currcenterid + '-x'" href="javascript:" class="btn btn-warning" v-link="{name: 'doctorgroup-importpatients',query:{'diseaseid': getDiseaseid()}}" ><i class="fa fa-sign-in fa">&nbsp;批量导入患者</i></a>
            </div>
        ...
        <table class="table table-bordered">
            ...
            <tr v-for="member in dg_center.members">
                ...
                <td class="linkstyle"><a class="scale" v-dg-privilege="projectid + '-1 | ' + member.center_id + '-x | clearlink'" href="javascript:" v-link="{name: 'doctorgroup-patientlist', query: {'projectid': projectid, 'centerid': currcenterid, 'doctorid': member.doctorid}}">{{member.patientcnt}}</a></td>
                <td class="linkstyle">
                    <a class="scale" v-dg-privilege="projectid + '-1 | ' + member.center_id + '-x'" href="javascript:" v-link="{name: 'doctorgroup-patientlist', query: {'projectid': projectid, 'centerid': currcenterid, 'doctorid': member.doctorid}}" style="padding: 5px; text-decoration: none">查看患者</a>
        ...
        <modal :show.sync="showModal" :showheader.sync="false" width="500px">
            <div slot="body">
        ...
        methods: {
            ...
            setprojectmaster: function(member) {
                ...
                $.ajax({
                    ...
                    if (response.errno == 0) {
                        self.$dispatch('show-popup', '设置成功', function() {
                            self.fetchData();
                        });
                    }else {
                        self.$dispatch('show-alert', response.errmsg);
            ...
            doSearch: function() {
                ...
                if (this.doctornameForSearch.trim() == '') {
                    // this.$dispatch('show-alert', '医生姓名不能为空', function(){
                ...
                if (this.doctormobileForSearch.trim() == '') {
                    // this.$dispatch('show-alert', '医生电话不能为空', function(){
            ...
            addmemberpost: function() {
                ...
                if (this.searchresultdoctor.name.trim() == '') {
                    this.$dispatch('show-alert', '请先搜索医生');
                    return ;
                }
                $.ajax({
