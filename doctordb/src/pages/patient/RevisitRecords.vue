    ...
    <div class="container-fluid content">
        ...
        <div v-else class="row">
            ...
            <ol class="breadcrumb" style="margin: 0">
                <li>返回</li>
                <li><a href="javascript:" v-link="{name: 'doctorgroup-projectlist'}" style="text-decoration: none">项目列表</a></li>
                <li><a href="javascript:" v-link="{name: 'doctorgroup-centerlist', params:{'projectid': projectid}}" style="text-decoration: none">中心列表</a></li>
                <li><a href="javascript:" v-link="{name: 'doctorgroup-centerdetail', params: {'projectid': projectid, 'centerid': centerid}}" style="text-decoration: none">中心详情</a></li>
                <li><a href="javascript:" v-link="{name: 'doctorgroup-patientlist', query: {'projectid': projectid, 'centerid': centerid, 'doctorid': doctorid}}" style="text-decoration: none">患者列表</a></li>
            </ol>
        </div>
        ...
        <div class="row" v-if="revisitrecords.length > 0">
            <div class="col-sm-12" v-for="revisitrecord in revisitrecords" style="padding:0" track-by="id">
                <template v-if="revisitrecord.typestr == '治疗'">
                    ...
                    <div class="checkup-data" v-if="chemoMaps[revisitrecord.id]" transition="toggle">
                            ...
                            <thead>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="mytd">{{{revisitrecord.pkg_items | formatePkgItems}}}</td>
                                        <td>{{revisitrecord.effect_name}}</td>
                                        <td>{{revisitrecord.effect_content}}</td>
                                        <td class="mytd">{{{revisitrecord.sideeffect_items | formteSideEffectItems}}}</td>
                                        <td class="mytd">{{{revisitrecord | formateXContent}}}</td>
                                    </tr>
                                </tbody>
                        </table>
                    </div>
                ...
                <template v-else>
                    <div class="header header-1" @click="clickHeader(revisitrecord, $event)">
                        <span class="fa" :class="{'fa-minus-square-o': dates[revisitrecord.thedate] == true, ' fa-plus-square-o': dates[revisitrecord.thedate] == false}" aria-hidden="true"></span><span>&nbsp;&nbsp;{{revisitrecord.thedate}} {{{revisitrecord.typestr | formateTypeStr}}}</span>
                        <span class="title" v-for="title in getCheckupTitles(revisitrecord)" track-by="$index">{{title}}</span>
                    </div>
                    <div class="checkup-data" v-if="dates[revisitrecord.thedate]" transition="toggle">
                        <template v-for="checkup in revisitrecord.checkup">
                            ...
                            <div class="container-fluid div1">
                                <div class="row trdiv" v-for="rowdata in getData4Row(checkup.xanswersheet.answers)">
                                    <div class="tddiv" v-for="answer in rowdata" :class="getClass($index, rowdata)">
                                        <span class="col-sm-6" :style="getStyle(1, $index, rowdata)">{{answer.xquestionname}}</span>
                                        <span class="col-sm-6" :style="getStyle(2, $index, rowdata)" @click="lookFullAnswer($event, answer)">
                                            {{{getContent(answer)}}}
    ...
    methods: {
        lookFullAnswer: function(e, answer) {
            if (e.target.scrollHeight > e.target.clientHeight) {
                ...
                this.$dispatch('show-alert', str, null, 'right')
            }
        },
