...
module.exports = {
    ...
    methods: {
        ...
        clickSave: function() {
            if ($.trim(this.name) == '') {
                this.$dispatch('show-alert', '名称不能为空');
                return false;
            }
            if ($.trim(this.username) == '') {
                this.$dispatch('show-alert', '登录名不能为空');
                return false;
            }
            if (this.mobile == '') {
                this.$dispatch('show-alert', '手机号不能为空');
                return false;
            }
            if (!rule.checkPhone(this.mobile)) {
                this.$dispatch('show-alert', '请输入正确的手机号');
                return false;
            }
            if (!this.ismodify) {
                if ($.trim(this.password1) == '') {
                    this.$dispatch('show-alert', '密码不能为空');
                    return false;
                }
                if (this.password1 != this.password2) {
                    this.$dispatch('show-alert', '两次输入的密码不一致');
                    return false;
                }
            }
