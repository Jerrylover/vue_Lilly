...
<div class="form-group question-group" v-if="isShowComponent">
    <div class="col-lg-7 col-sm-7 row">
        <label for="inputEmail3" class="col-lg-3 col-sm-4 control-label question-label" :a="className">{{{question.content}}}</label>
        <div class="col-lg-9 col-sm-8">
            <div class="segmented-control" style="width:100%;border:1px solid #ccc">
                ...
                <label v-for="option in question.options" for="inlineRadio{{option.id}}" _id='{{checkuptpl.ename}}-{{question.ename}}-{{option.content}}' :_class='_class' data-value="{{option.content}}" @click="clickLabel(option, $event)" :class="{active1: optionActive(option)}"
    ...
    <div class="col-sm-2 select" v-if="isShowQualitatives">
        ...
        <span class="" v-else>{{{qualitatives[0]}}}</span>
    </div>
    ...
    methods: {
        ...
        clickLabel: function(option, e) {
            ...
            if (popover.is(':hidden')) {
                this.showPopover = false;
                this.$dispatch('edss-hide-popover');
            }
    ...
    watch: {
        'fs': function(newval, oldval) {
            if (this.isEdss) {
                ...
                this.$dispatch('edss-fs-change', this.question.content, newval);
            }
        }
    },
    ready: function() {
