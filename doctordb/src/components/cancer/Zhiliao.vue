            ...
            <div class="form-group question-group">
                ...
                <div class="col-sm-3" style="">
                    <div :class="{'has-error has-feedback': iserror, 'has-feedback': !iserror}">
                        ...
                        <calendar :show.sync="show" :value.sync="startdate" :x="x" :y="y" :begin="begin" :end="end" :range="range"></calendar>
                    </div>
                </div>
            </div>
            <div class="form-group question-group">
                ...
                <div class="col-sm-3" style="">
                    <div>
                        <select class="form-control" v-model="pickedPkgname" @change="pkgnameChange()">
                          <option v-for="pkgname in pkgnames" :value="pkgname" track-by="$index">{{pkgname}}</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group question-group">
                <div class="col-sm-12 text-center">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr v-for="(index, medicine) in medicines" track-by="_id">
                                ...
                                <template v-else >
                                    ...
                                    <td>
                                            ...
                                            <option v-for="method in medicine.method" track-by="$index"  :value="method">{{method}}</option>
                                            </select>
                                    </td>
                                    <td><select class="form-control" v-model="medicine.pickedmethod2">
                                    <option v-for="method in medicine.method2" track-by="$index" :value="method">{{method}}</option>
                                </select> </td>
                                    <td width="15%">
                                        <input class="form-control" type="text" v-model="medicine.method3" :value="medicine.method3" />
                                    </td>
                                    <td width="15%">
                                        ...
                                        <option v-for="method in medicine.method4" track-by="$index"  :value="method">{{method}}</option>
                                    </select>
                                    </td>
                                    <td width="15%">
                                            <select class="form-control" v-model="medicine.pickedtime">
                                      <option v-for="time in medicine.time" track-by="$index"  :value="time">{{time}}</option>
                                    </select>
                                    </td>
                                    <td width="15%">
                                            <input type="text" class="form-control" v-model="medicine.remark" />
                    ...
                    <div class="col-sm-5" id="search-medicine-div" style="font-size:14px;" v-show="isShowSearchMedicineDiv">
                        <div style="max-height:200px;overflow-y:auto;overflow-x:hidden" @mouseenter="doMouseenter($event)" @mouseleave="doMouseleave($event)">
                            <table class="table table-bordered" style="border:0">
                                ...
                                <tbody>
                                    <tr v-for="one in searchlist" v-if="searchlist.length > 0" track-by="$index">
                ...
                <div v-for="arr in arr2" class="form-group question-group" class="div-badreaction" track-by="$index">
                    ...
                    <div class="col-sm-3" style="">
                        <div>
                            <select class="form-control" v-model="arr[0]">
                              <option v-for="(key, val) in badreactions" :value="key">{{key}}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group question-group">
                        ...
                        <div class="col-sm-3" style="">
                            <div :class="{'has-error has-feedback': iserror, 'has-feedback': !iserror}">
                                ...
                                <span class="help-block" v-show="iserror">日期已存在</span>
                                <calendar :show.sync="xshow" :value.sync="xstartdate" :x="x" :y="y" :begin="begin" :end="end" :range="range"></calendar>
                            </div>
                        </div>
                    ...
                    <thead>
                        ...
                        <tbody>
                            <tr v-for="chemo in chemos">
                                ...
                                <td class="mytd">{{{chemo.pkg_items | formatePkgItems}}}</td>
                                <td>{{chemo.effect_name}}</td>
                                <td>{{chemo.effect_content}}</td>
                                <td class="mytd">{{{chemo.sideeffect_items | formteSideEffectItems}}}</td>
                                <td class="mytd">{{{chemo | formateXContent}}}</td>
...
module.exports = {
    ...
    methods: {
        ...
        chemoRemove: function(chemo) {
            var that = this;
            this.$dispatch('show-prompt', '确定要删除该量表吗？', function() {
                $.ajax({
