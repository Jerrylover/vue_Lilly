            ...
            <div class="text-center">
                <div class="div-year">
                    <div style="float:left">
                        <input type="text" v-model.number="year" @change="render(year,month)" min="1900" max="2200" maxlength="4">
                    </div>
                    <div class="div-triangle">
                        <div class="triangle-up" @click="year++"></div>
                        <div class="triangle-down" @click="year--"></div>
                    </div>
                </div>
                <span>年&nbsp;&nbsp;{{month+1}}&nbsp;&nbsp;月</span>
            </div>
        </div>
        <table cellpadding="5">
            <thead>
            ...
            <tr v-for="(k1,day) in days">
                <td v-for="(k2,child) in day" :class="{'today':child.today,'disabled':child.disabled}" @click="select(k1,k2,$event)" @touchstart="select(k1,k2,$event)">
                    {{child.day}}
...
export default {
    ...
    created: function() {
        ...
        if (that.value != "") {
            ...
            var value = '';
            if (that.defaultdate == false) {
                value = "";
            }else{
                value = that.output([that.year, that.month, that.day]);
            }
            that.$emit('calendar-change-value', value)
        }
    ...
    methods: {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (that.range) {
                    ...
                    if (
                        ...
                        // 当天
                    } else if (chkY == that.year && chkM == that.month && i == that.day) {
                        temp[line].push({
                            day: i,
                            today: true
                        });
        ...
        select: function(k1, k2, e) {
            ...
            if (that.range) {
                ...
                that.today = [k1, k2];
                if (that.type == 'date') {
                    var value = that.year + that.sep + that.zero(that.month + 1) + that.sep + that.zero(that.days[k1][k2].day);
                    that.$emit('calendar-change-value', value)
                    that.show = false;
                }
            }
        ...
        ok: function() {
            ...
            var value = '';
            if (that.range) {
                value = that.output(that.rangeBegin) + " ~ " + that.output(that.rangeEnd);
            } else {
                value = that.output([that.year, that.month, that.day, that.hour, that.minute, that.second]);
            }
            that.$emit('calendar-change-value', value)
            that.show = false;
        },
